...
#include "DilateNode.hpp"


const NodeDesc DilateNode::desc =
{
    "DilateNode",
    "DilateNode",
    CAT_PROC_MORPH,
...
DilateNode::DilateNode()
    :BaseNodeModel(DilateNode::desc)
{
}

void DilateNode::process()
{
    ...
    try {
        cv::dilate(inputMat, _output, kernel,
                  cv::Point(-1, -1),
                  _iterations,
                  _borderType);
    }
    ...
    {
        qDebug("cv::Dilate failed: %s", e.what());
        _output = inputMat.clone();   // fail-safe
    }
