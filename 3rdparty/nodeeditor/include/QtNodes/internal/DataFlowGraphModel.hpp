...
public:
    ...
    bool moveNodeInto(DataFlowGraphModel &dst, NodeId nid);
    bool moveConnectionInto(DataFlowGraphModel &dst, ConnectionId const &cid);
    void clearGraph();
    // 迁移中使用：无需检查合法性
    void addConnectionDirect(ConnectionId const &cid);

    void deleteConnectionDirect(ConnectionId const &cid);

    void notifyConnectionCreated(ConnectionId const &cid);
