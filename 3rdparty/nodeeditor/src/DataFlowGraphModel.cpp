...
bool DataFlowGraphModel::moveNodeInto(DataFlowGraphModel &dst, NodeId nid)
{
    // 1) move model
    auto it = _models.find(nid);
    if (it != _models.end()) {
        dst._models[nid] = std::move(it->second);
        _models.erase(it);
    } else
        return false;

    // 2) move geometry
    auto git = _nodeGeometryData.find(nid);
    if (git != _nodeGeometryData.end()) {
        dst._nodeGeometryData[nid] = git->second;
        _nodeGeometryData.erase(git);
    }

    return true;
}

bool DataFlowGraphModel::moveConnectionInto(DataFlowGraphModel &dst, ConnectionId const &cid)
{
    auto cit = _connectivity.find(cid);
    if (cit != _connectivity.end()) {
        dst._connectivity.insert(*cit);
        _connectivity.erase(cit);
        return true;
    }
    return false;
}

void DataFlowGraphModel::clearGraph()
{
    _connectivity.clear();
    _models.clear();
    _nodeGeometryData.clear();
    //_nodeGroupingInfo.clear(); // ������з���ϵͳ
}

void DataFlowGraphModel::addConnectionDirect(ConnectionId const& cid)
{
    _connectivity.insert(cid);
}

void DataFlowGraphModel::deleteConnectionDirect(ConnectionId const& cid)
{
    auto it = _connectivity.find(cid);
    if (it != _connectivity.end())
    {
        _connectivity.erase(it);
    }
...
void DataFlowGraphModel::notifyConnectionCreated(ConnectionId const& cid)
{
    Q_EMIT sendConnectionCreation(cid);
}
